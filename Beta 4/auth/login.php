...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE BINARY c.usuario=? AND c.activo=1 LIMIT 1
    ");
    $stmt->execute([$usuario]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    // Verificación de seguridad: el usuario debe coincidir EXACTAMENTE (case-sensitive)
    if ($user && $user['usuario'] === $usuario && password_verify($contrasena, $user['contrasena'])) {
