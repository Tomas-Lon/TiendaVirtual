    ...
    switch ($action) {
        case 'create':
            try {
                ...
                // Iniciar transacción para crear empleado y credencial
                ...
                if ($success) {
                    ...
                    // Crear credencial automáticamente con usuario basado en nombre
                    $usuario = strtolower(str_replace(' ', '.', $_POST['nombre']));
                    
                    // Validar y ajustar usuario si es duplicado
                    $usuario_base = $usuario;
                    $contador = 1;
                    while (true) {
                        $check = $pdo->prepare("SELECT id FROM credenciales WHERE usuario = ?");
                    ...
                    // Insertar credencial con cliente_id = NULL explícitamente
                    $cred_stmt = $pdo->prepare("INSERT INTO credenciales (usuario, contrasena, tipo, empleado_id, cliente_id, activo) VALUES (?, ?, 'empleado', ?, NULL, ?)");
                    $cred_stmt->execute([
                        $usuario,
                        $password_hash,
                        $empleado_id,
                    ...
                    $mensaje = "Empleado creado exitosamente. Usuario: <strong>$usuario</strong> | Contraseña temporal: <strong>$password_temporal</strong>";
                ...
                if (strpos($e->getMessage(), 'Duplicate') !== false) {
                    $mensaje = 'Error: Ya existe un empleado con ese email o usuario';
                } else {
        ...
        case 'update':
            try {
                ...
                // Iniciar transacción para actualizar empleado y credencial
                $pdo->beginTransaction();
                ...
                if ($success) {
                    // Actualizar el estado activo de la credencial del empleado
                    $cred_stmt = $pdo->prepare("UPDATE credenciales SET activo = ? WHERE empleado_id = ?");
                    $cred_stmt->execute([
                        isset($_POST['activo']) ? 1 : 0,
                        $_POST['id']
                    ]);
        ...
        case 'delete':
            try {
                ...
                if ($has_clients) {
                    ...
                    // Eliminar empleado (la credencial se borra automáticamente por CASCADE)
                    $stmt = $pdo->prepare("DELETE FROM empleados WHERE id = ?");
                    $stmt->execute([$_POST['id']]);
                    $mensaje = 'Empleado y su credencial eliminados exitosamente';
                    $tipo_mensaje = 'success';
                }
            } catch (PDOException $e) {
