...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
		...
		try {
                ...
                if ($action === 'buscar_productos') {
                        ...
                        if ($search === '' && $grupo_id === '') {
                                $sql = "SELECT p.id, p.codigo, p.descripcion, p.precio, p.unidad_medida, p.unidad_empaque, 
                                               0 AS stock_disponible, g.nombre AS grupo_nombre
						...
						// Obtener solo direcciones activas y de envío
						$stmt = $pdo->prepare("SELECT id, direccion, ciudad, departamento, codigo_postal FROM direcciones_clientes WHERE cliente_id = ? AND activo = 1 AND es_envio = 1 ORDER BY es_principal DESC, id DESC");
                ...
                if ($action === 'crear_cotizacion' || $action === 'crear_pedido') {
                        ...
                        $metodo_pago = trim($_POST['metodo_pago'] ?? 'contado');
                        ...
                        if ($action === 'crear_pedido' && $fecha_entrega) {
                                $min = new DateTime($fechaMinimaAdmin);
                                $fe = DateTime::createFromFormat('Y-m-d', $fecha_entrega);
                                if ($fe && $fe < $min) {
                        ...
                        try {
                                ...
                                foreach ($productos as $prod) {
                                        $pid = (int)($prod['id'] ?? 0);
                                        $qty = (int)($prod['cantidad'] ?? 0);
                                        if ($pid <= 0 || $qty <= 0) continue;
                                        
                                        // Obtener precio actual del producto
                                        $stmt = $pdo->prepare('SELECT precio, grupo_id FROM productos WHERE id = ?');
                                        $stmt->execute([$pid]);
                                        $prodInfo = $stmt->fetch(PDO::FETCH_ASSOC);
                                        if (!$prodInfo) continue;
                                        
                                        $precio = (float)$prodInfo['precio'];
                                        $grupo_id = $prodInfo['grupo_id'];
                                        
                                        // Buscar descuento por grupo del cliente
                                        $desc_grupo = 0;
                                        if ($grupo_id) {
                                                $stmt = $pdo->prepare('SELECT porcentaje_descuento FROM descuentos_clientes WHERE cliente_id = ? AND grupo_id = ? AND activo = 1');
                                                $stmt->execute([$cliente_id, $grupo_id]);
                                                $descInfo = $stmt->fetch(PDO::FETCH_ASSOC);
                                                if ($descInfo) {
                                                        $desc_grupo = (float)$descInfo['porcentaje_descuento'];
                                                }
                                        }
                                        
                                        $subtotal_item = $qty * $precio;
                                        ...
                                        $productosConPrecio[] = [
                                                ...
                                                'subtotal' => $subtotal_item,
                                                'descuento_porcentaje' => $desc_grupo,
                                                'descuento_monto' => $subtotal_item * ($desc_grupo / 100)
                                        ];
                                }
                                
                                if (empty($productosConPrecio)) {
                                ...
                                $impuestos = $base_gravable * 0.19; // IVA 19%
                                $total = $base_gravable + $impuestos;
                                
                                // Obtener empleado_id de la sesión
                                $empleado_id = $_SESSION['user_id'] ?? null;
                                
                                // Insertar pedido (sin metodo_pago ni persona_recibe - no existen en la tabla)
                                $estado = $action === 'crear_pedido' ? 'confirmado' : 'borrador';
                                
                                // Agregar info de método de pago y persona que recibe en observaciones
                                $obs_completas = $observaciones;
                                if ($metodo_pago && $metodo_pago !== 'contado') {
                                ...
                                $stmt = $pdo->prepare('
                                        INSERT INTO pedidos (numero_documento, cliente_id, empleado_id, direccion_entrega_id, 
                                                           fecha_pedido, fecha_entrega_estimada, estado, subtotal, descuento_porcentaje, 
                                                           descuento_total, impuestos_total, total, observaciones)
                                        VALUES (?, ?, ?, ?, NOW(), ?, ?, ?, ?, ?, ?, ?, ?)
                                ');
                                $stmt->execute([
                                        $numero, 
                                ...
                                foreach ($productosConPrecio as $item) {
                                        $stmt->execute([
                                                $pedido_id,
                                                $item['producto_id'],
                                                $item['cantidad'],
                                ...
                                $pdo->rollBack();
                                error_log('Error creando pedido/cotización: ' . $e->getMessage());
			...
			<div class="card shadow-sm sticky-top" style="top: 20px;">
				...
				<div class="card-body">
					...
					<div class="alert alert-info p-2 mb-3"><small>
						<div class="d-flex justify-content-between"><span>IVA (19%):</span><span id="iva_total">$0.00</span></div>
		...
		let html=''; cart.forEach((p,id)=>{ html+=`
			<div class="border rounded p-2 mb-2 bg-light">
				<div class="d-flex justify-content-between align-items-start">
					<div><strong>${escapeHtml(p.descripcion)}</strong><br><small class="text-muted">Código: ${escapeHtml(p.codigo)} | ${escapeHtml(p.unidad_medida||'und')} | Emp: ${p.unidad_empaque||1}</small></div>
...
function mostrarProductos(productos){ const container=document.getElementById('productos_encontrados'); if(!productos || productos.length===0){ container.innerHTML='<div class="alert alert-info mb-0">No se encontraron productos</div>'; return; } let html='<div class="card"><div class="card-header bg-light d-flex justify-content-between align-items-center" style="cursor:pointer" onclick="toggleResultados()"><span><i class="fas fa-list me-2"></i><strong>Resultados de búsqueda</strong></span><i class="fas fa-chevron-down" id="iconToggleResultados"></i></div><div class="search-results show" id="listaResultados">'; productos.forEach(p=>{ const codigo=escapeHtml(p.codigo); const desc=escapeHtml(p.descripcion); const grupo=escapeHtml(p.grupo_nombre||'Sin grupo'); const uMed=escapeHtml(p.unidad_medida||'und'); const uEmp=p.unidad_empaque||1; html+=`<div class="search-result-item" onclick="agregarDesdeBusqueda(${p.id}, '${codigo}', '${desc}', ${p.precio}, '${uMed}', ${uEmp})"><div class="d-flex justify-content-between align-items-start"><div><strong>${desc}</strong><br><small class="text-muted">Código: ${codigo} | U.Med: ${uMed} | U.Emp: ${uEmp}</small><br><small class="text-info">Grupo: ${grupo}</small></div><div class="text-end"><div class="fw-bold text-primary">$${formatCurrency(p.precio)}</div><small class="text-muted">Stock: ${p.stock_disponible||0}</small></div></div></div>`; }); html+='</div></div>'; container.innerHTML=html; }
...
function agregarDesdeBusqueda(id,codigo,descripcion,precio,uMed,uEmp){ const qty=Math.max(1,parseInt(document.getElementById('cantidad').value)||1); const ex=cart.get(id); if(ex){ ex.cantidad+=qty; } else { cart.set(id,{ id, codigo, descripcion, precio:Number(precio), cantidad:qty, unidad_medida:uMed||'und', unidad_empaque:uEmp||1 }); } renderCart(); document.getElementById('search_producto').value=''; document.getElementById('productos_encontrados').innerHTML=''; }
...
function calcularTotales(){ const subtotal=Array.from(cart.values()).reduce((s,it)=>s+(it.cantidad*it.precio),0); const dPct=Math.max(0,Math.min(100,parseFloat(document.getElementById('descuento_porcentaje').value)||0)); const dMonto=subtotal*(dPct/100); const base=subtotal-dMonto; const iva=base*0.19; const total=base+iva; document.getElementById('subtotal').textContent='$'+formatCurrency(subtotal); document.getElementById('iva_total').textContent='$'+formatCurrency(iva); document.getElementById('base_gravable').textContent='$'+formatCurrency(base); document.getElementById('total_pagar').textContent='$'+formatCurrency(total); }
