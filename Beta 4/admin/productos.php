...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="btn-group">
        ...
        <ul class="dropdown-menu">
            <li><a class="dropdown-item" href="#" onclick="exportarLista('pdf'); return false;">
                <i class="fas fa-file-pdf text-danger"></i> Exportar PDF
            </a></li>
            <li><a class="dropdown-item" href="#" onclick="exportarLista('xlsx'); return false;">
                <i class="fas fa-file-excel text-success"></i> Exportar Excel
            </a></li>
        </ul>
        <button type="button" class="btn btn-primary ms-2" data-bs-toggle="modal" data-bs-target="#productModal">
...
$additionalJS = <<<EOT
<script>
    // Función para exportar lista completa de productos
    function exportarLista(formato) {
        ...
        link.href = url;
        link.download = 'lista_productos_' + new Date().toISOString().split('T')[0] + '.' + (formato === 'xlsx' ? 'csv' : formato);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        
        // Mostrar mensaje de éxito
        const toastHtml = '<div class="toast align-items-center text-white bg-success border-0" role="alert" aria-live="assertive" aria-atomic="true" style="position: fixed; top: 20px; right: 20px; z-index: 9999;">' +
            '<div class="d-flex">' +
            '<div class="toast-body"><i class="fas fa-check-circle me-2"></i>Exportando lista de productos...</div>' +
            '<button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast"></button>' +
            '</div></div>';
        
        const toastEl = document.createElement('div');
        toastEl.innerHTML = toastHtml;
        document.body.appendChild(toastEl.firstElementChild);
