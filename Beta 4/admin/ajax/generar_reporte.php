...
if ($formato === 'excel') {
    // Recolectar filas y usar CSVExporter para formateo consistente
    if ($tipo === 'ventas') {
        $headers = ['Fecha', 'Total Pedidos', 'Total Ventas', 'Promedio Venta'];
        $rows = [];
        $sql = "SELECT 
        ...
        while ($row = $stmt->fetch()) {
            $rows[] = [
                $row['fecha'],
                intval($row['total_pedidos']),
                (float)$row['total_ventas'],
                (float)$row['promedio_venta']
            ];
        }
        CSVExporter::rawExport($headers, $rows, 'reporte_ventas_' . date('Y-m-d') . '.csv');

    } elseif ($tipo === 'productos') {
        $headers = ['Código', 'Descripción', 'Cantidad Vendida', 'Total Ingresos', 'Pedidos'];
        $rows = [];
        $sql = "SELECT 
                    p.codigo,
                    p.descripcion,
        ...
        while ($row = $stmt->fetch()) {
            $rows[] = [
                $row['codigo'],
                $row['descripcion'],
                intval($row['total_vendido']),
                (float)$row['total_ingresos'],
                intval($row['pedidos_count'])
            ];
        }
        CSVExporter::rawExport($headers, $rows, 'reporte_productos_' . date('Y-m-d') . '.csv');

    } elseif ($tipo === 'clientes') {
        $headers = ['Cliente', 'Email', 'Total Pedidos', 'Total Comprado'];
        $rows = [];
        $sql = "SELECT 
                    c.nombre,
                    c.email,
        ...
        while ($row = $stmt->fetch()) {
            $rows[] = [
                ...
                $row['email'],
                intval($row['total_pedidos']),
                (float)$row['total_comprado']
            ];
        }
        CSVExporter::rawExport($headers, $rows, 'reporte_clientes_' . date('Y-m-d') . '.csv');
    }
}
