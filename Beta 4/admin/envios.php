...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        if ($_POST['action'] === 'crear') {
            ...
            $direccionEntregaId = isset($_POST['direccion_entrega_id']) && $_POST['direccion_entrega_id'] !== '' ? (int)$_POST['direccion_entrega_id'] : null;
...
$pedidos_sin_envio_sql = "SELECT p.id, p.numero_documento as numero, c.id as cliente_id, c.nombre as cliente, 
                          p.direccion_entrega_id, p.fecha_entrega_estimada
                          FROM pedidos p 
                          INNER JOIN clientes c ON p.cliente_id = c.id 
        ...
        <div class="modal-content">
            <form id="formEnvio">
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" id="fecha_programada_hidden" name="fecha_programada">
                    
                    <div class="row g-3">
                        <div class="col-md-8">
                            <label for="pedido_id" class="form-label fw-semibold">Pedido *</label>
                            <select class="form-select" id="pedido_id" required>
                                ...
                                <?php foreach ($pedidos_sin_envio as $pedido): ?>
                                    <option value="<?php echo $pedido['id']; ?>" 
                                            ...
                                            data-direccion-entrega-id="<?php echo $pedido['direccion_entrega_id'] ? $pedido['direccion_entrega_id'] : ''; ?>"
                                            data-fecha-entrega="<?php echo $pedido['fecha_entrega_estimada'] ? $pedido['fecha_entrega_estimada'] : ''; ?>">
                        ...
                        <div class="col-md-4">
                            <label for="numero_guia" class="form-label fw-semibold">Número de Guía</label>
                        ...
                        <div class="col-md-6">
                            <label for="fecha_programada" class="form-label fw-semibold">Fecha Programada *</label>
                            <div class="input-group">
                                <input type="date" class="form-control" id="fecha_programada" required>
                                <button class="btn btn-outline-warning" type="button" id="btnCambiarFecha" style="display:none" title="Cambiar fecha de entrega">
                                    <i class="fas fa-edit"></i> Cambiar
                                </button>
                            </div>
                            <div id="fecha_original_info" class="alert alert-success mt-2 py-2 px-3" style="display:none">
                                <small>
                                    <i class="fas fa-check-circle"></i> 
                                    <strong>Fecha seleccionada del pedido:</strong> 
                                    <span id="fecha_original_texto"></span>
                                </small>
                            </div>
                            <div id="fecha_cambio_advertencia" class="alert alert-warning mt-2 py-2 px-3" style="display:none">
                                <small>
                                    <i class="fas fa-exclamation-triangle"></i> 
                                    <strong>¡Atención!</strong> Estás modificando la fecha del pedido.
                                    <br>
                                    <span class="text-muted">Fecha original: <span id="fecha_original_texto_advertencia"></span></span>
                                </small>
                            </div>
                        </div>
                        
                        <div class="col-md-6">
                            <label for="direccion_entrega_id" class="form-label fw-semibold">Dirección de Entrega *</label>
                            <input type="hidden" id="direccion_entrega_id_hidden" name="direccion_entrega_id">
                            <div class="input-group">
                                <select class="form-select" id="direccion_entrega_id" required disabled>
                                    <option value="">Seleccione primero un pedido</option>
                                </select>
                                <button class="btn btn-outline-warning" type="button" id="btnCambiarDireccion" style="display:none" title="Cambiar dirección de entrega">
                                    <i class="fas fa-edit"></i> Cambiar
                                </button>
                            </div>
                            <div id="direccion_original_info" class="alert alert-success mt-2 py-2 px-3" style="display:none">
                                <small>
                                    <i class="fas fa-check-circle"></i> 
                                    <strong>Dirección del pedido:</strong> 
                                    <span id="direccion_original_texto"></span>
                                </small>
                            </div>
                            <div id="direccion_cambio_advertencia" class="alert alert-warning mt-2 py-2 px-3" style="display:none">
                                <small>
                                    <i class="fas fa-exclamation-triangle"></i> 
                                    <strong>¡Atención!</strong> Estás modificando la dirección del pedido.
                                    <br>
                                    <span class="text-muted">Dirección original: <span id="direccion_original_texto_advertencia"></span></span>
                                </small>
                            </div>
                        </div>
                        
                        <div class="col-md-6">
                            <label for="receptor_nombre" class="form-label fw-semibold">Receptor</label>
                            <input type="text" class="form-control" id="receptor_nombre" name="receptor_nombre" 
                                   placeholder="Nombre de quien recibe">
                        </div>
                        <div class="col-md-6">
                            <label for="receptor_documento" class="form-label fw-semibold">Documento Receptor</label>
                            <input type="text" class="form-control" id="receptor_documento" name="receptor_documento" 
                                   placeholder="CC/NIT">
                        </div>
                        
                        <div class="col-md-6">
                            <label for="repartidor_id" class="form-label fw-semibold">Repartidor</label>
                        ...
                        <div class="col-md-6">
                            <label for="estado_select" class="form-label fw-semibold">Estado</label>
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control" id="observaciones" name="observaciones" rows="2" placeholder="Notas adicionales sobre el envío..."></textarea>
                        </div>
                    </div>
                </div>
...
document.getElementById("fecha_programada").value = fechaHoy;
document.getElementById("fecha_programada_hidden").value = fechaHoy;

// Sincronizar fecha cuando cambie
document.getElementById("fecha_programada").addEventListener("change", function() {
    document.getElementById("fecha_programada_hidden").value = this.value;
});
...
if (pedidoSelectEl && numeroGuiaEl && direccionSelectEl) {
    let direccionesData = [];
    let direccionOriginalId = null;
    let direccionOriginalNombre = "";
    let fechaOriginal = null;
    let fechaOriginalTexto = "";
    let modoEdicion = false;
    
    pedidoSelectEl.addEventListener("change", function() {
        // Sincronizar con campo hidden
        document.getElementById("pedido_id_hidden").value = this.value;
        
        const opt = this.options[this.selectedIndex];
        const numeroDoc = opt ? opt.getAttribute("data-numero-documento") : "";
        const clienteId = opt ? opt.getAttribute("data-cliente-id") : "";
        const direccionEntregaId = opt ? opt.getAttribute("data-direccion-entrega-id") : "";
        const fechaEntrega = opt ? opt.getAttribute("data-fecha-entrega") : "";
        ...
        // Ocultar botones y alertas de dirección
        document.getElementById("btnCambiarDireccion").style.display = "none";
        document.getElementById("direccion_original_info").style.display = "none";
        document.getElementById("direccion_cambio_advertencia").style.display = "none";
        
        // Ocultar botones y alertas de fecha
        document.getElementById("btnCambiarFecha").style.display = "none";
        document.getElementById("fecha_original_info").style.display = "none";
        document.getElementById("fecha_cambio_advertencia").style.display = "none";
        
        // Precargar y bloquear fecha de entrega si existe
        const fechaProgramadaEl = document.getElementById("fecha_programada");
        if (fechaProgramadaEl && fechaEntrega) {
            fechaProgramadaEl.value = fechaEntrega;
            document.getElementById("fecha_programada_hidden").value = fechaEntrega;
            fechaOriginal = fechaEntrega;
            
            // Formatear fecha para mostrar
            const fechaObj = new Date(fechaEntrega + 'T00:00:00');
            fechaOriginalTexto = fechaObj.toLocaleDateString('es-CO', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
            });
            
            document.getElementById("fecha_original_texto").textContent = fechaOriginalTexto;
            document.getElementById("fecha_original_info").style.display = "block";
            
            // Bloquear el input y mostrar botón de cambio
            fechaProgramadaEl.disabled = true;
            document.getElementById("btnCambiarFecha").style.display = "inline-block";
        } else {
            fechaProgramadaEl.disabled = false;
            fechaOriginal = null;
        }
            ...
            fetch("ajax/get_direcciones_cliente.php?cliente_id=" + clienteId)
                ...
                .then(data => {
                    ...
                    if (data.success && data.direcciones) {
                        ...
                        // Preseleccionar la dirección que tiene el pedido
                        if (direccionEntregaId) {
                            direccionSelectEl.value = direccionEntregaId;
                            document.getElementById("direccion_entrega_id_hidden").value = direccionEntregaId;
                            direccionOriginalId = direccionEntregaId;
                            
                            // Obtener nombre de la dirección original
                            const dirOriginal = data.direcciones.find(d => d.id == direccionEntregaId);
                            if (dirOriginal) {
                                direccionOriginalNombre = dirOriginal.nombre + " - " + dirOriginal.ciudad;
                                document.getElementById("direccion_original_texto").textContent = direccionOriginalNombre;
                                document.getElementById("direccion_original_info").style.display = "block";
                            }
                            
                            // Bloquear el select y mostrar botón de cambio
                            direccionSelectEl.disabled = true;
                            document.getElementById("btnCambiarDireccion").style.display = "inline-block";
                            
                            // Disparar el evento change para cargar los datos del receptor
                            direccionSelectEl.dispatchEvent(new Event('change'));
                        } else {
    ...
    // Botón para habilitar cambio de dirección
    document.getElementById("btnCambiarDireccion").addEventListener("click", function() {
        if (confirm("⚠️ ATENCIÓN: Vas a cambiar la dirección de entrega\n\nDirección actual (del pedido): " + direccionOriginalNombre + "\n\n¿Deseas continuar con el cambio?")) {
            direccionSelectEl.disabled = false;
            this.style.display = "none";
            document.getElementById("direccion_original_info").style.display = "none";
            document.getElementById("direccion_cambio_advertencia").style.display = "block";
            document.getElementById("direccion_original_texto_advertencia").textContent = direccionOriginalNombre;
            modoEdicion = true;
        }
    });
    
    // Botón para habilitar cambio de fecha
    document.getElementById("btnCambiarFecha").addEventListener("click", function() {
        if (confirm("⚠️ ATENCIÓN: Vas a cambiar la fecha de entrega\n\nFecha actual (del pedido): " + fechaOriginalTexto + "\n\n¿Deseas continuar con el cambio?")) {
            const fechaProgramadaEl = document.getElementById("fecha_programada");
            fechaProgramadaEl.disabled = false;
            this.style.display = "none";
            document.getElementById("fecha_original_info").style.display = "none";
            document.getElementById("fecha_cambio_advertencia").style.display = "block";
            document.getElementById("fecha_original_texto_advertencia").textContent = fechaOriginalTexto;
            
            // Agregar listener para confirmar cambio de fecha
            fechaProgramadaEl.addEventListener("change", function() {
                if (this.value && this.value !== fechaOriginal) {
                    const nuevaFechaObj = new Date(this.value + 'T00:00:00');
                    const nuevaFechaTexto = nuevaFechaObj.toLocaleDateString('es-CO', { 
                        year: 'numeric', 
                        month: 'long', 
                        day: 'numeric' 
                    });
                    if (!confirm("⚠️ ATENCIÓN: Estás cambiando la fecha de entrega.\n\nFecha original: " + fechaOriginalTexto + "\nNueva fecha: " + nuevaFechaTexto + "\n\n¿Estás seguro de aplicar este cambio?")) {
    ...
    direccionSelectEl.addEventListener("change", function() {
        // Sincronizar con campo hidden
        document.getElementById("direccion_entrega_id_hidden").value = this.value;
...
function editarEnvio(envio) {
    ...
    document.getElementById("pedido_id_hidden").value = envio.pedido_id;
    ...
    if (envio.cliente_id) {
        ...
        const direccionOriginalId = envio.direccion_entrega_id;
            ...
            .then(data => {
                ...
                if (data.success && data.direcciones) {
                    data.direcciones.forEach(dir => {
                        ...
                        option.dataset.contactoReceptor = dir.contacto_receptor || "";
                        option.dataset.documentoReceptor = dir.documento_receptor || "";
                        if (dir.id == direccionOriginalId) {
                            option.selected = true;
                            direccionOriginalNombre = dir.nombre + " - " + dir.ciudad;
                        }
                        direccionSelect.appendChild(option);
                    });
                    
                    // Mostrar info de dirección original
                    if (direccionOriginalNombre) {
                        document.getElementById("direccion_original_texto").textContent = direccionOriginalNombre;
                        document.getElementById("direccion_original_info").style.display = "block";
                    }
                    
                    // Listener para detectar cambios en la dirección
                    direccionSelect.addEventListener("change", function() {
                        if (this.value && this.value != direccionOriginalId) {
                            if (!confirm("⚠️ ATENCIÓN: Estás cambiando la dirección de entrega.\n\nDirección original: " + direccionOriginalNombre + "\nNueva dirección: " + this.options[this.selectedIndex].text + "\n\n¿Estás seguro de aplicar este cambio?")) {
                                this.value = direccionOriginalId;
                            }
                        }
                    }, { once: false });
                }
...
document.getElementById("modalEnvio").addEventListener("hidden.bs.modal", function() {
    ...
    document.getElementById("pedido_id_hidden").value = "";
    document.getElementById("numero_guia").readOnly = true;
    ...
    document.getElementById("direccion_entrega_id").disabled = true;
    document.getElementById("direccion_entrega_id_hidden").value = "";
    document.getElementById("direccion_original_info").style.display = "none";
    document.getElementById("direccion_cambio_advertencia").style.display = "none";
    document.getElementById("btnCambiarDireccion").style.display = "none";
    const fechaHoy = new Date().toISOString().split("T")[0];
    document.getElementById("fecha_programada").value = fechaHoy;
    document.getElementById("fecha_programada_hidden").value = fechaHoy;
    document.getElementById("fecha_programada").disabled = false;
    document.getElementById("fecha_original_info").style.display = "none";
    document.getElementById("fecha_cambio_advertencia").style.display = "none";
    document.getElementById("btnCambiarFecha").style.display = "none";
});

// Auto-abrir modal si viene desde pedidos
document.addEventListener('DOMContentLoaded', function() {
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('crear_envio') === '1') {
        const pedidoData = sessionStorage.getItem('pedido_crear_envio');
        if (pedidoData) {
            try {
                const data = JSON.parse(pedidoData);
                // Verificar que no sea muy antiguo (max 5 minutos)
                if (Date.now() - data.timestamp < 300000) {
                    // Seleccionar el pedido
                    const pedidoSelect = document.getElementById('pedido_id');
                    if (pedidoSelect) {
                        pedidoSelect.value = data.pedido_id;
                        // Disparar el evento change para cargar direcciones
                        pedidoSelect.dispatchEvent(new Event('change'));
                ...
                sessionStorage.removeItem('pedido_crear_envio');
            } catch (e) {
                console.error('Error al cargar datos del pedido:', e);
            }
        }
    }
});
