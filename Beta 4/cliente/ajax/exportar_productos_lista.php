<?php
/**
 * Endpoint AJAX para exportar lista completa de productos (Cliente)
...
function exportarPDF($productos) {
    ...
    $pdf->Cell(0, 8, 'Total de productos en catálogo: ' . $contador, 0, 1, 'L');
    
    // Footer en la última página
    $pdf->SetY(-15);
    $pdf->SetFont('Arial', 'I', 8);
    $pdf->SetTextColor(128, 128, 128);
    $pdf->Cell(0, 10, 'Página ' . $pdf->PageNo() . ' - Catálogo de Productos', 0, 0, 'C');
    
    // Generar descarga
    $nombre_archivo = 'catalogo_productos_' . date('Y-m-d_H-i-s') . '.pdf';
...
function exportarXLSX($productos) {
    $nombre_archivo = 'catalogo_productos_' . date('Y-m-d_H-i-s') . '.csv';
    
    // Usar el exportador centralizado para formato consistente
    ...
    $rows = array_map(function($p){
        return [
            ...
            $p['descripcion'],
            $p['grupo'] ?? 'Sin grupo',
            isset($p['precio']) ? (float)$p['precio'] : null,
            $p['unidad_medida'] ?? 'und',
            $p['unidad_empaque'] ?? 1
        ];
    }, $productos);

    \CSVExporter::rawExport($headers, $rows, $nombre_archivo);
}
