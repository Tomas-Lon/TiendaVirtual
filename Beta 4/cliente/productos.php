                        ...
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-sm btn-primary w-100" 
                                        onclick="openAddToCartModal(<?= (int)$producto['id'] ?>, '<?= htmlspecialchars(addslashes($producto['codigo'])) ?>', '<?= htmlspecialchars(addslashes($producto['descripcion'])) ?>', <?= (float)$producto['precio'] ?>)">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="alert alert-info">
                    <i class="fas fa-info-circle"></i> Ingrese la cantidad de unidades que desea agregar al carrito
                </div>
                
                <div class="mb-3">
                    <label class="form-label fw-bold"><i class="fas fa-box"></i> Producto</label>
                    <div class="p-3 bg-light rounded border">
                        <table class="table table-sm table-borderless mb-0">
                            <tr>
                                <td class="text-muted" style="width: 40%;"><strong>Código:</strong></td>
                                <td><span id="modal_producto_codigo"></span></td>
                            </tr>
                            <tr>
                                <td class="text-muted"><strong>Descripción:</strong></td>
                                <td><span id="modal_producto_descripcion"></span></td>
                            </tr>
                            <tr>
                                <td class="text-muted"><strong>Precio unitario:</strong></td>
                                <td><strong class="text-primary">$<span id="modal_producto_precio"></span></strong></td>
                            </tr>
                        </table>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    <label for="modal_cantidad" class="form-label"><i class="fas fa-hashtag"></i> Cantidad *</label>
                    <input type="number" class="form-control form-control-lg" id="modal_cantidad" 
                           min="1" step="1" value="1" required 
                           placeholder="Ingrese cantidad">
                    <small class="text-muted">Cantidad mínima: 1 unidad</small>
                </div>
                
                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-calculator"></i> Subtotal</label>
                    <div class="alert alert-success mb-0">
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="fw-bold">Total a agregar:</span>
                            <h4 class="mb-0 text-success">$<span id="modal_subtotal">0.00</span></h4>
                        </div>
                    </div>
                </div>
            </div>
...
function openAddToCartModal(productId, codigo, descripcion, precio) {
    // Guardar datos del producto en el modal
    document.getElementById('modal_producto_id').value = productId;
    document.getElementById('modal_producto_codigo').textContent = codigo;
    document.getElementById('modal_producto_descripcion').textContent = descripcion;
    document.getElementById('modal_producto_precio').textContent = Number(precio).toFixed(2);
    document.getElementById('modal_cantidad').value = 1;
    currentProductPrice = Number(precio);
    ...
    // Focus en cantidad
    setTimeout(() => {
        document.getElementById('modal_cantidad').select();
    }, 500);
}

function updateSubtotal() {
    const cantidad = Number(document.getElementById('modal_cantidad').value) || 1;
    const subtotal = currentProductPrice * cantidad;
...
document.addEventListener('DOMContentLoaded', function() {
    const cantidadInput = document.getElementById('modal_cantidad');
    if (cantidadInput) {
        cantidadInput.addEventListener('input', updateSubtotal);
        cantidadInput.addEventListener('change', updateSubtotal);
    }
    
    // Botón confirmar agregar
    document.getElementById('btnConfirmarAgregar').addEventListener('click', function() {
        const productId = document.getElementById('modal_producto_id').value;
        const cantidad = Number(document.getElementById('modal_cantidad').value);
    ...
    document.getElementById('modal_cantidad').addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            document.getElementById('btnConfirmarAgregar').click();
        }
    });
});
...
function addToCart(productId, cantidad) {
    ...
    .then(data => {
        if (data && data.success) {
            ...
            showFlash(`✅ ${cantidad} unidad(es) agregada(s). Abre "Nueva Compra" para revisar.`, 'success');
        } else {
