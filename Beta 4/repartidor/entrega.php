...
<body>
    ...
    <div class="container">
        ...
        <form id="entregaForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-section step-2">
                ...
                <div class="info-box">
                    <i class="fas fa-exclamation-triangle text-danger"></i> <strong>Requerido:</strong> Debes tomar una foto del cliente con el paquete o del paquete entregado como evidencia. Sin foto no se puede completar la entrega.
                </div>
            ...
            <div class="form-section d-grid gap-2">
                ...
                <button type="button" class="btn btn-warning btn-lg" id="btnMarcarFallida">
                    <i class="fas fa-exclamation-triangle"></i> <strong>Marcar como Fallida</strong>
                </button>
                <button type="button" class="btn btn-danger btn-lg" id="btnCancelarEntrega">
                    <i class="fas fa-ban"></i> <strong>Cancelar Entrega</strong>
                </button>
    ...
    <div class="modal fade" id="modalFallida" tabindex="-1" aria-labelledby="modalFallidaLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-warning text-dark">
                    <h5 class="modal-title" id="modalFallidaLabel">
                        <i class="fas fa-exclamation-triangle"></i> Marcar Entrega como Fallida
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle"></i> Esta entrega <strong>podrá reintentarse</strong> más tarde desde el panel de entregas.
                    </div>
                    <div class="mb-3">
                        <label for="motivoFallida" class="form-label">Motivo del fallo *</label>
                        <select class="form-select" id="motivoFallida" required>
                    ...
                    <div class="mb-3">
                        <label for="observacionesFallida" class="form-label">Observaciones adicionales</label>
                        <textarea class="form-control" id="observacionesFallida" rows="3" 
                                  placeholder="Ej: Vecino indica que regresa a las 5pm, Portero no permitió acceso, etc."></textarea>
                        <small class="text-muted">Agrega cualquier detalle relevante sobre el intento fallido</small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                    <button type="button" class="btn btn-warning" id="btnConfirmarFallida">
                        <i class="fas fa-exclamation-triangle"></i> Marcar como Fallida
                    </button>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title" id="modalCancelarLabel">
                        <i class="fas fa-ban"></i> Cancelar Entrega Definitivamente
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-danger">
                        <i class="fas fa-exclamation-circle"></i> <strong>ADVERTENCIA:</strong> Esta entrega <strong>NO podrá reintentarse</strong>. Se marcará como devuelta definitivamente.
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="motivoCancelacion" required>
                            ...
                            <option value="Cliente no desea recibir">Cliente no desea recibir</option>
                            <option value="Otro">Otro (especificar abajo)</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="detallesCancelacion" class="form-label">Detalles de la cancelación</label>
                        <textarea class="form-control" id="detallesCancelacion" rows="3" 
                                  placeholder="Explica los detalles de por qué se cancela definitivamente esta entrega..."></textarea>
                        <small class="text-muted">Proporciona información clara sobre la cancelación</small>
                    </div>
                    <div class="form-check mb-3">
                        ...
                        <label class="form-check-label" for="confirmarCancelacion">
                            <strong>Confirmo que esta entrega debe cancelarse y NO puede reintentarse</strong>
                        </label>
                    </div>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" id="btnConfirmarCancelacion">
                        <i class="fas fa-ban"></i> Cancelar Entrega
                    </button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('btnMarcarFallida').addEventListener('click', marcarFallida);
            document.getElementById('btnCancelarEntrega').addEventListener('click', cancelarEntrega);
        ...
        // Marcar como fallida (puede reintentarse)
        function marcarFallida() {
            // Limpiar formulario
            document.getElementById('motivoFallida').value = '';
            document.getElementById('observacionesFallida').value = '';
            
            // Mostrar modal
            const modal = new bootstrap.Modal(document.getElementById('modalFallida'));
            modal.show();
        }

        // Confirmar marcar como fallida
        document.getElementById('btnConfirmarFallida').addEventListener('click', function() {
            const motivo = document.getElementById('motivoFallida').value.trim();
            const observaciones = document.getElementById('observacionesFallida').value.trim();

            if (!motivo) {
                alert('Por favor selecciona un motivo del fallo.');
                document.getElementById('motivoFallida').focus();
                return;
            }

            // Cerrar modal
            const modal = bootstrap.Modal.getInstance(document.getElementById('modalFallida'));
            modal.hide();

            // Mostrar spinner
            const spinner = document.getElementById('loadingSpinner');
            spinner.style.display = 'block';
            document.getElementById('btnMarcarFallida').disabled = true;
            ...
            .then(data => {
                ...
                if (data.success) {
                    alert('✅ ENTREGA MARCADA COMO FALLIDA\n\nLa entrega ha sido marcada como fallida.\nPodrás reintentarla desde el panel de entregas.\n\nRedirigiendo al dashboard...');
                    setTimeout(() => {
                        window.location.href = 'dashboard.php';
                    }, 1000);
                } else {
                    alert('❌ ERROR: ' + (data.message || 'Error desconocido'));
                    document.getElementById('btnMarcarFallida').disabled = false;
        ...
        // Cancelar entrega definitivamente (no se puede reintentar)
        function cancelarEntrega() {
            // Limpiar formulario
            document.getElementById('motivoCancelacion').value = '';
            document.getElementById('detallesCancelacion').value = '';
        ...
        document.getElementById('btnConfirmarCancelacion').addEventListener('click', function() {
            ...
            const detalles = document.getElementById('detallesCancelacion').value.trim();
            ...
            if (!confirmado) {
                alert('Debes confirmar que la entrega debe cancelarse definitivamente.');
                document.getElementById('confirmarCancelacion').focus();
                return;
            }
            ...
            fetch('ajax/cambiar_estado.php', {
                ...
                body: JSON.stringify({
                    ...
                    observaciones: detalles || 'Cancelación definitiva desde formulario de entrega'
                })
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.success) {
                    alert('✅ ENTREGA CANCELADA\n\nLa entrega ha sido cancelada definitivamente.\nEsta entrega NO puede reintentarse.\n\nRedirigiendo al dashboard...');
                    setTimeout(() => {
                        window.location.href = 'dashboard.php';
                    }, 1000);
                } else {
        ...
        function guardarEntrega(event) {
            ...
            if (!fotoCapturada) {
                alert('⚠️ FOTO OBLIGATORIA\n\nDebes capturar una foto de evidencia antes de completar la entrega.');
                document.getElementById('btnCapturarFoto').scrollIntoView({ behavior: 'smooth' });
                document.getElementById('btnCapturarFoto').classList.add('btn-danger');
                setTimeout(() => {
                    document.getElementById('btnCapturarFoto').classList.remove('btn-danger');
                }, 2000);
                return;
            }

            if (signaturePad.isEmpty()) {
                alert('⚠️ FIRMA OBLIGATORIA\n\nPor favor captura la firma del cliente antes de continuar.');
