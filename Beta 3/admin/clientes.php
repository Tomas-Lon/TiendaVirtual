...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            ...
            // Iniciar transacción para crear cliente y credencial
            ...
            $usuario = strtolower(str_replace(' ', '.', $clienteData['nombre']));
            // Si el usuario ya existe, agregar un sufijo aleatorio
            ...
            // Insertar credencial con empleado_id = NULL explícitamente
            $cred_stmt = $pdo->prepare("INSERT INTO credenciales (usuario, contrasena, tipo, cliente_id, empleado_id, activo) VALUES (?, ?, 'cliente', ?, NULL, ?)");
            $cred_stmt->execute([
                $usuario,
                $password_hash,
                $cliente_id,
            ...
            $mensaje = "Cliente creado exitosamente. Usuario: <strong>$usuario</strong> | Contraseña temporal: <strong>$password_temporal</strong>";
        ...
        if ($action === 'update') {
            ...
            // Iniciar transacción para actualizar cliente y credencial
            $pdo->beginTransaction();
            ...
            // Actualizar el estado activo de la credencial del cliente
            $cred_stmt = $pdo->prepare("UPDATE credenciales SET activo = ? WHERE cliente_id = ?");
            $cred_stmt->execute([
                $clienteData['activo'],
                $id
            ]);
        ...
        if ($action === 'delete') {
            ...
            if ($has_orders) {
                ...
                // Eliminar cliente (la credencial se borra automáticamente por CASCADE)
                $stmt = $pdo->prepare("DELETE FROM clientes WHERE id = :id");
                $stmt->execute([':id' => $id]);
                $mensaje = 'Cliente y su credencial eliminados exitosamente';
                $tipo_mensaje = 'success';
            }
        }
    } catch (Exception $e) {
