...
if ($busqueda) {
    ...
    $where_conditions[] = "(c.nombre LIKE ? OR p.numero_documento LIKE ? OR e.direccion_entrega LIKE ?)";
...
if ($fecha_desde) {
    $where_conditions[] = 'DATE(e.fecha_entrega) >= ?';
    $params[] = $fecha_desde;
}
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <?php if ($entrega['estado'] === 'programado'): ?>
                                        ...
                                        <a href="entrega.php?id=<?= $entrega['id'] ?>" class="btn btn-success" title="Completar">
                                    ...
                                    <?php elseif ($entrega['estado'] === 'entregado'): ?>
                                        <?php if ($entrega['comprobante_pdf']): ?>
                                            <button class="btn btn-success" onclick="verComprobante('<?= addslashes($entrega['comprobante_pdf']) ?>')" title="Ver comprobante">
                                    ...
                                    <?php elseif ($entrega['estado'] === 'fallido'): ?>
                                        ...
                                        <button class="btn btn-warning" onclick="reintentar(<?= $entrega['id'] ?>)" title="Reintentar">
...
function verDetallesEntrega(entrega) {
    ...
    if (entrega.estado === 'entregado' && entrega.comprobante_pdf) {
        htmlDetalles += '<div class="alert alert-success mt-3">' +
            ...
            '<button type="button" class="btn btn-success btn-sm" onclick="verComprobante(\'' + entrega.comprobante_pdf.replace(/'/g, "\\'") + '\'); bootstrap.Modal.getInstance(document.getElementById(\'modalDetallesEntrega\')).hide();">' +
...
function reintentar(entregaId) {
    if (!confirm('¿Reintentar esta entrega? El estado cambiará a "Programado".')) return;
    
    fetch('ajax/cambiar_estado.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({entrega_id: entregaId, estado: 'programado'})
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            alert('✓ Entrega reprogramada correctamente');
            location.reload();
        } else {
            alert('Error: ' + data.message);
        }
